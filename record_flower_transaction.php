...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($geneticsId) || empty($weight) || empty($transactionType) || empty($reason)) {
        header("Location: record_dry_weight.php?error=All fields are required");
        exit();
    }

    // If the reason is 'Other', ensure 'otherReason' is provided
    if ($reason === 'Other' && empty($otherReason)) {
        header("Location: record_dry_weight.php?error=Please provide the 'Other' reason");
        exit();
    }
    ...
    if ($transactionType === 'Subtract' && ($reason === 'Testing' || $reason === 'Send external') && empty($companyId)) {
        header("Location: record_dry_weight.php?error=Please select a company for Testing or Send external transactions");
        exit();
    }
    ...
    if ($stmt->execute()) {
        header("Location: record_dry_weight.php?success=Flower transaction recorded successfully");
    } else {
        header("Location: record_dry_weight.php?error=Error recording transaction: " . $conn->error);
    }
