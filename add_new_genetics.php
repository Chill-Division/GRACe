...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($geneticsName)) {
        ...
        header("Location: add_new_genetics.html?error=Genetics name is required&data=$data");
        exit();
    }
    ...
    if ($conn->query($sql) === TRUE) {
        ...
        header("Location: add_new_genetics.html?success=New genetics added successfully");
        exit();
    } else {
        // Redirect with error message and submitted data
        $data = json_encode($_POST); // Encode submitted data as JSON
        header("Location: add_new_genetics.html?error=Error adding genetics&data=$data");
        exit();
    }
}
