...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($plantCount) || empty($geneticsId)) {
        ...
        header("Location: receive_genetics.php?error=Plant count and genetics name are required&data=$data");
        exit();
    }
    ...
    for ($i = 0; $i < $plantCount; $i++) {
        ...
        if (!$conn->query($sql)) {
            ...
            $data = json_encode($_POST);
            header("Location: receive_genetics.php?error=Error inserting plants: " . $conn->error . "&data=$data");
            exit();
        }
    }

    // Redirect with success message
    header("Location: receive_genetics.php?success=Genetics received successfully");
    exit();
}
